...
3. (Optional) Generate audio samples for the sample-based instruments. Requires **ffmpeg** and **bc** (`brew install ffmpeg`):
   ...
   Without this step, only "Base Sound" (hidden synth instrument) will work. The active instruments **MKS Drone** and **MKS Realistic** both require the samples generated by `generate-mks-samples.sh`.
